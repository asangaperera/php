...
<h1><p align="center"><b><u>RETURN PAGE</u></b></p></h1>
<h2><p align="center"><b><u>You can return items which you bought within 7 days only</u></b></p></h2>
<?php
...
<?php
	...
	$sql = "SELECT COUNT(*) FROM cart , nautoitem, users WHERE cart.cust_id = $userid AND users.user_id = $userid AND cart.checkout = 'y' AND nautoitem.id = cart.nauto_id";
	$query = (mysqli_query($db_conx,$sql));
	...
	$selectproducts = "SELECT * FROM cart , nautoitem, users WHERE cart.cust_id = $userid AND users.user_id = $userid AND cart.checkout = 'y' AND nautoitem.id = cart.nauto_id";
	...
	else
	{
		echo "<table align='center' cellspacing='1' cellpadding='0' width='80%'>
				...
				<th align='center' bgcolor='e6e6e6'>RETURN</th>
				";
		...
		for($loop = 0; $loop < $countrows; $loop++)
		{
			...
			while($row = mysqli_fetch_array($query, MYSQLI_ASSOC))
			{
				...
				<td>" . $amount . "</td>";
				echo $list;
			echo '<td><form action="returnpro.php" method="post" onSubmit="return submitForms()" "returninPosInteger()">
						<input type="hidden" name="txtid" value= '. $userid.'">
						<input type="hidden" name="txtamount" value= '. $amount.'">
						<input type="hidden" name="txtprodname" value= '. $prodname.'">
						<input type="hidden" name="txtid" value='.$foodid. '">
...
<br 
<h2><center>
<p><b>THANK YOU FOR CHOSING</b><h1> vegi.top</h1><b> ONLINE SUPERMARKET<b></p>
</center></h2>
</body>
</html>/>
