<?php
	...
	$sql = " SELECT * FROM nautoitem ORDER BY RAND( ) " . $limit ;
	...
	else
	{
		...
		$category = $row["category"];
		$price = "Rs. " . $row["price"];
		$type ="Brand Name:-". $row["type"];
		$mfgdate = "MFG. Date:-".$row["mfgdate"];
		$exdate = "EXP. Date:-".$row["exdate"];
		$sp_make=$row["sp_make"];
		$width="150px";
		$height="150px";
		...
		<div>
		 ...
		 $list .='
		   <div class="proName">
			<div class="name"><a href="' . $src . $image . '">' . $prodname . '</a></div>
			<div class="sp_make">' . $sp_make . '</div>
			<div class="type">' . $type . '</div>
			<div class="mfgdate">' . $mfgdate . '</div>
