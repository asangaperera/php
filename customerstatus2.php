	...
	else
{
   ...
   $sql = "UPDATE users SET address='$Address', tel='$Tel' WHERE user_id='$userid'"; 
             $result =  mysql_query($sql) or die(mysql_error());

   $que = mysqli_query($db_conx,$sql);

    // If result matched $myusername and $mypassword, table row must be 1 row
    if($que)
	{
		echo "<script>alert('We will find you...!')</script>";
	}
	else
	{
...
<form method="post" action="">
	  <input type="submit" id="Button" align="right" name="sub" class="btn btn-primary btn-xs"  value= " GET mRECEIPT ">
  </form>
 </div>
