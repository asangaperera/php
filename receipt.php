...
<p align="right"><a href="index.php">Home</a> | <a href="logout.php">Logout</a></p>
...
<?php
	...
	$sql = "SELECT COUNT(*) FROM cart , nautoitem, users WHERE cart.cust_id = $userid AND users.user_id = $userid AND cart.checkout = 'y' AND nautoitem.id = cart.nauto_id AND cart.checkedon = '$today' AND cart.trans = '$code'";
	...
	$selectproducts = "SELECT * FROM cart , nautoitem, users WHERE cart.cust_id = $userid AND users.user_id = $userid AND cart.checkout = 'y' AND nautoitem.id = cart.nauto_id AND cart.checkedon = '$today' AND cart.trans = '$code'";
	...
	else
	{
		...
		for($loop = 0; $loop < $countrows; $loop++)
		{
			...
			while($row = mysqli_fetch_array($query, MYSQLI_ASSOC))
			{
				...
				$vat = round(0.00 * $subtotal);
				$totalamount = ($subtotal + $vat + $delivery);
				
				$amount = round($amount);

                $sql = "INSERT INTO cart SET amount = '$amount' WHERE id = '$id'";
	
$sql = "INSERT INTO total( nauto_id, cust_id,qty, added, checkedon, trans,amount, status) VALUES ( '$foodid', '$userid','$qty','$added' , '$today', '$code','$amount', Redy )";
...
<p>THANK YOU FOR CHOSING </p><h1>Vegi.top</h1>
