...
$form_password = $_POST['txtpassword'];
	...
	while ($row = $dbquery->fetcharray())
	{
		...
		else if(($status == "2") AND ($ac_type == "cook"))
		{
			$user_id = $row[0];
			$username = $row[1];
			echo "<script>alert('Welcome Back to seller page')</script>";
			echo "<script>window.location.href='adminarea/seller.php';</script>";
		}
		else if(($status == "3") AND ($ac_type == "staff"))
		{
			$user_id = $row[0];
			$username = $row[1];
			echo "<script>alert('Welcome Back to staff page')</script>";
			echo "<script>window.location.href='adminarea/staff.php';</script>";
		}
